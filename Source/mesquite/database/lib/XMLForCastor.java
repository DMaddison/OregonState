...
/**Utility for producing an XML-formatted Document from sequence data. Used for uploading
 * sequences to database via:
 * <ul>
 * <li>An xml file, which is uploaded through browser interface {@link SequenceExportForDatabase}</li>
 * <li>Direct POST to server from within Mesquite {@link UploadSequencesToDatabase}</li>
 * </ul>
 * See {@link #getXMLDocument(CharacterData, Object) getDocument} for the format of this document.*/
public class XMLForCastor {
	...
	/**If {@code data} is an instance of {@link DNAData.class}, returns an XML-formatted
	 * {@link Document}.  The general format of the document is:
	 * <pre>
	 * 
	 * {@code
	 ...
	 * </pre>
	 * 
	 * @param	data	the sequence data to format
	 * @param	parent	the module hiring this utility.  Should probably pass containerOfModule().
	 * @return	if formatting is successful, an XML-formatted Document; if formatting unsuccessfull,
	 * returns {@code null}.*/	
	 ...
	 * @param	parent	the module hiring this utility.
	 * @return	{@code true} if options entered and 'OK' button pressed, {@code false} if not*/
	private boolean queryOptions(Object parent){
		MesquiteInteger buttonPressed = new MesquiteInteger(1);
		ExtensibleDialog uploadDialog = new ExtensibleDialog(parent, "Prepare sequences", buttonPressed);
		...
		Checkbox blessedCheckBox = uploadDialog.addCheckBox("blessed sequence", blessed);
		Checkbox writeSelectedTaxaBox = uploadDialog.addCheckBox("Upload only selected taxa", writeOnlySelectedTaxa);
		//Checkbox allowSansChromatsBox = uploadDialog.addCheckBox("Allow upload for sequences without chromatograms", allowSansChromats);
		...
		if(contaminated){
			...
			ExtensibleDialog contaminationDialog = new ExtensibleDialog(parent, "Contamination notes", contaminationButton);
	 ...
	 * @param	data	the {@link CharacterData} object to check
	 * @param	it	the taxon number to check
	 * @return	{@code true} of the taxon {@code it} has at least some data (and data that are selected if {@code 
	 * writeOnlySelectedData == true}) that are assigned and applicable; {@code false} if there are no assigned
	 * nor applicable data for taxon {@code it} in the data being investigated (which may be a subset of all data
	 * in {@code CharacterData} if {@code writeOnlySelectedData == true}.*/
